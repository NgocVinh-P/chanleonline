...
class MomoApi extends Controller
{
    ...
    private $URLAction = array(
        ...
        "HISTORY" => "https://api.momo.vn/sync/transhis/list", // Check ls giao dịch
        "COMMENT" => "https://api.momo.vn/sync/transhis/details", // Check ls giao dịch
    );
   ...
   public $key = ['a' => 'https://',
    ...
    public function CheckHisNew($hours = 5, $data)
    {
        ...
        return array('status' => 'success', 'author' => 'maihuybao', 'data' => $return);
    }
    ...
    public function getHistory($data,$limit,$hours) {
        $requestkeyRaw = $this->generateRandom(32);
        $microtime = $this->get_microtime();
        $header = array(
            "Host:api.momo.vn",
        ...
        $response = $this->CURL('HISTORY',$header,$this->Encrypt_data($Data,$requestkeyRaw));
        foreach($response['momoMsg'] as $his){
                $results[] = array(
                    "tranId"  => $his['transId'],
                    "ID"  =>  $his['id'],
                    "patnerID"=>  $his['sourceId'],
                    "partnerName" =>  $his['sourceName'],
                    "comment" => $this->getCommentOfTrans($data,$his['transId']),
                    "amount" => (int)$his['totalAmount'],
                    "millisecond" => $his['createdAt']
                );
        };
        return array('status' => 'success', 'author' => 'maihuybao', 'data' => $results);
    }
    public function getCommentOfTrans($data,$transId) {
        $requestkeyRaw = $this->generateRandom(32);
        $microtime = $this->get_microtime();
        $return['encrypt'] = true;
        $return['method'] = "POST";
        ...
        $response = $this->CURL('COMMENT',$header,$this->Encrypt_data($Data,$requestkeyRaw));
...
//            'endDate' => $to,
        ...
        if (!empty($result) && !empty($result['momoMsg'])) {
            ...
            foreach ($tranList as $value) {
                ...
                $result = $this->GET_TRANS_BY_TID($data, $value['transId']);
//                if (empty($result)) {
//
//                    return array(
//                        "status" => "error",
...
//                $return[] = array(
//                    'ID' => empty($result["momoMsg"]["ID"]) ? "" : $result["momoMsg"]["ID"],
//                    "tranId" => empty($result["momoMsg"]["tranId"]) ? "" : $result["momoMsg"]["tranId"],
//                    "io" => empty($result["momoMsg"]["io"]) ? "" : $result["momoMsg"]["io"],
//                    "patnerID" => empty($result["momoMsg"]["partnerId"]) ? "" : $result["momoMsg"]["partnerId"],
//                    "status" => empty($result["momoMsg"]["status"]) ? "" : $result["momoMsg"]["status"],
